CREATE PROCEDURE [Transform].[GetFactInternetSalesChangedKeys](
	@DebugOnly bit = 0
)
AS
BEGIN
	...
	WHERE ([PropertyID] = 'FactInternetSalesVersion');
	...
	IF (@LastTransformedVersion IS NOT NULL)
		BEGIN
			...
			IF EXISTS(
				SELECT * FROM [Transform].[FactInternetSalesChangedKeys]
			)
				RETURN(0);
			...
			INSERT INTO [Transform].[FactInternetSalesChangedKeys]([OrderDate] ,[SalesOrderID], [SalesOrderDetailID], [RevisionNumber])
	...
	ELSE
		BEGIN
			TRUNCATE TABLE [Transform].[FactInternetSalesChangedKeys];
